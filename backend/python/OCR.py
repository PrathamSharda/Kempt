...
def cleanup_directory_contents(directory_path):
    """Clean up all contents of a directory without removing the directory itself"""
    try:
...
def download_from_gcs(gcs_path):
    try:
        ...
        if not ensure_directory_exists(file_storage_dir):
            raise Exception(f"Failed to create directory: {file_storage_dir}")
        
        # Create file path in FileStorage
        base_filename = os.path.basename(blob_name)
        if not base_filename:
            base_filename = "downloaded_file.pdf"
        file_path = os.path.join(file_storage_dir, base_filename)
...
async def pdf_to_image(file_path):
    ...
    try:
        ...
        images_dir = "/FileStorage/Images"
        if not ensure_directory_exists(images_dir):
            return {}
        
        for i, page in enumerate(pages):
            pdf_name = os.path.splitext(os.path.basename(file_path))[0]
            name_image = os.path.join(images_dir, f'{pdf_name}_page_{i+1}.jpg')
            page.save(name_image, 'JPEG')
...
async def call():
    ...
    if not ensure_directory_exists(file_storage_dir):
        return "Failed to create FileStorage directory."
    
    if not ensure_directory_exists(images_dir):
        return "Failed to create FileStorage/Images directory."
    ...
    try:
        ...
        # Create PDF in FileStorage directory
        pdf_name = os.path.splitext(os.path.basename(temp_file_path))[0]
        output_pdf_path = os.path.join(file_storage_dir, f"{pdf_name}_extracted.pdf")
        ...
        while os.path.exists(output_pdf_path):
            ...
            output_pdf_path = f"{name}_{counter}{ext}"
            counter += 1
        ...
        # Clean up downloaded PDF and all images
        cleanup_temp_file(temp_file_path)
        cleanup_directory_contents("/FileStorage/Images")
